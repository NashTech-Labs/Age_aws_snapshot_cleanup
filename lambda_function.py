            ...
            try:
                ...
                for region in ec2_regions:  
                    ...
                    for snapshot in snapshots['Snapshots']:
                        ...
                        try:
                            if difference_of_days.days>30 :             # specify the difference in days here....already specified value is 30
                                id = snapshot['SnapshotId']
                                print(id) 
                                client.delete_snapshot(SnapshotId=id)      # this line deletes the snapshot...
        ...
        if acc_id[i]==master_acc_id:
            ...
            try:
                ...
                for region in ec2_regions:    
                    ...
                    # use snapshots = client.describe_snapshots() in place of line 252 when not using tags      
                    # snapshots = client.describe_snapshots(Filters=[{'Name': 'tag:Team', 'Values': ['Devops']}])   an example if u want to use filters
                    snapshots = client.describe_snapshots()      
                    print(snapshots)
                    for snapshot in snapshots['Snapshots']:
                        start_time=snapshot['StartTime']
                        ...
                        today_date=datetime.datetime.now().date()
                        difference_of_days=today_date-date
                        print(difference_of_days.days)   
                        
                        try:
                            if difference_of_days.days>30 :          # specify the difference of days here.....already specified value is 30
                                id = snapshot['SnapshotId']
                                print(id)          
                                client.delete_snapshot(SnapshotId=id)    # this line deletes the snapshot...        
                        except botocore.exceptions.ClientError as e:
